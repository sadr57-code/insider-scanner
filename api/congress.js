// api/congress.js — Political trades via QuiverQuant API
// Simple, fast, works within Vercel Hobby 10s limit
// Redis cache: 1hr TTL
// Add QUIVER_API_KEY to Vercel env vars — $30/month at api.quiverquant.com
...
async function fetchQuiver(ticker) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`QuiverQuant ${r.status}: ${text.slice(0, 200)}`);
  }

  const raw = await r.json();
  return (Array.isArray(raw) ? raw : []).map(normalizeQuiver).filter(Boolean);
}

// Normalize QuiverQuant response fields:
// Representative, Ticker, Transaction, Range, House, Party,
// TransactionDate, ReportDate, Amount, ExcessReturn, PriceChange
function normalizeQuiver(t) {
  if (!t) return null;
  ...
  return {
    id:             `${t.Representative}-${t.Ticker}-${tradeDate}`,
...
function getMockTrades(tickerFilter) {
  ...
  const all = [
    ...
    { id:'9',  representative:'Shelley Capito',     ticker:'XOM',  transaction:'Buy',  range:'$15,001 - $50,000',     amount:15001,  chamber:'Senate', party:'R', tradeDate:ago(32), reportDate:ago(1),  reportingGap:31, lateFlag:true,  excessReturn:'+2.3%',  priceChange:'+4.1%'  },
    { id:'10', representative:'David Rouzer',       ticker:'BA',   transaction:'Buy',  range:'$100,001 - $250,000',   amount:100001, chamber:'House',  party:'R', tradeDate:ago(35), reportDate:ago(2),  reportingGap:33, lateFlag:true,  excessReturn:'+11.2%', priceChange:'+14.8%' },
    { id:'11', representative:'Kevin Hern',         ticker:'CVX',  transaction:'Sell', range:'$50,001 - $100,000',    amount:50001,  chamber:'House',  party:'R', tradeDate:ago(8),  reportDate:ago(6),  reportingGap:2,  lateFlag:false, excessReturn:'-3.7%',  priceChange:'-1.2%'  },
    { id:'12', representative:'Lois Frankel',       ticker:'NFLX', transaction:'Buy',  range:'$100,001 - $250,000',   amount:100001, chamber:'House',  party:'D', tradeDate:ago(13), reportDate:ago(11), reportingGap:2,  lateFlag:false, excessReturn:'+13.7%', priceChange:'+17.2%' },
    { id:'13', representative:'Roger Wicker',       ticker:'NOC',  transaction:'Buy',  range:'$50,001 - $100,000',    amount:50001,  chamber:'Senate', party:'R', tradeDate:ago(14), reportDate:ago(13), reportingGap:1,  lateFlag:false, excessReturn:'+8.1%',  priceChange:'+10.9%' },
    { id:'14', representative:'Donald J. Trump',    ticker:'DJT',  transaction:'Buy',  range:'$1,000,001 - $5,000,000',amount:1000001,chamber:'N/A',  party:'R', tradeDate:ago(45), reportDate:ago(10), reportingGap:35, lateFlag:true,  excessReturn:null,     priceChange:null     },
    { id:'15', representative:'Virginia Foxx',      ticker:'UNH',  transaction:'Sell', range:'$50,001 - $100,000',    amount:50001,  chamber:'House',  party:'R', tradeDate:ago(28), reportDate:ago(27), reportingGap:1,  lateFlag:false, excessReturn:'-8.2%',  priceChange:'-5.1%'  },
  ];
