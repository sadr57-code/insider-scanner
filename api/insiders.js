// api/insiders.js — Insider trades via SEC EDGAR ATOM feed + company submissions API
// No IP blocking issues — official SEC endpoints
...
// ─── Fetch Form 4 filings from EDGAR ATOM feed ───────────────────────────────
// This endpoint is official, public, and not IP-blocked
async function fetchEdgarAtom(days) {
  const cutoff = new Date(Date.now() - parseInt(days) * 86400000);
  
  // EDGAR full-text search — correct endpoint (not search-index)
  const url = `https://efts.sec.gov/LATEST/search-index?q=%22P+-+Purchase%22&forms=4&dateRange=custom&startdt=${cutoff.toISOString().split('T')[0]}&hits.hits.total=true`;
...
// ─── Alternative: use EDGAR company search ATOM feed ────────────────────────
async function fetchEdgarRSS() {
  // Latest Form 4 filings RSS — always works, no auth needed
  ...
  for (const entry of entries) {
    ...
    // Extract CIK and accession from the entry id URL
    const cikMatch = id.match(/CIK=(\d+)/i) || id.match(/data\/(\d+)\//);
    const accMatch = id.match(/accession-number=([0-9-]+)/i);
    
    if (!cikMatch || !accMatch) continue;
...
async function parseForm4(cik, accNo, date) {
  try {
    ...
    const cleanAcc = String(accNo).replace(/-/g, '');
...
const IND = {
  ...
  MU:'Technology',LRCX:'Technology',KLAC:'Technology',MRVL:'Technology',HPQ:'Technology',
  HPE:'Technology',DELL:'Technology',IBM:'Technology',CSCO:'Technology',ANET:'Technology',
  ...
  TEAM:'Technology',HUBS:'Technology',ZM:'Technology',VEEV:'Technology',PAYC:'Technology',
  CDNS:'Technology',SNPS:'Technology',ANSS:'Technology',ACN:'Technology',CTSH:'Technology',
  TSM:'Technology',ASML:'Technology',ARM:'Technology',MCHP:'Technology',MPWR:'Technology',
  AMZN:'Technology',SHOP:'Technology',CMCSA:'Technology',TMUS:'Technology',VZ:'Technology',
  T:'Technology',TTD:'Technology',ROKU:'Technology',NFLX:'Technology',
  ...
  SYK:'Healthcare',BSX:'Healthcare',MDT:'Healthcare',EW:'Healthcare',BAX:'Healthcare',
  BDX:'Healthcare',BIIB:'Healthcare',ALNY:'Healthcare',NVAX:'Healthcare',BNTX:'Healthcare',
  HCA:'Healthcare',CNC:'Healthcare',HUM:'Healthcare',CVS:'Healthcare',MCK:'Healthcare',
  HOLX:'Healthcare',DXCM:'Healthcare',WBA:'Healthcare',JAZZ:'Healthcare',
  ...
  APO:'Financials',ARES:'Financials',MET:'Financials',PRU:'Financials',AFL:'Financials',
  ALL:'Financials',TRV:'Financials',CB:'Financials',AIG:'Financials',
  ...
  PSX:'Energy',VLO:'Energy',MPC:'Energy',HES:'Energy',DVN:'Energy',HAL:'Energy',
  NEE:'Energy',DUK:'Energy',SO:'Energy',AEP:'Energy',EXC:'Energy',FSLR:'Energy',
  TSLA:'Consumer',HD:'Consumer',MCD:'Consumer',NKE:'Consumer',SBUX:'Consumer',
  LOW:'Consumer',TJX:'Consumer',ROST:'Consumer',F:'Consumer',GM:'Consumer',
  BKNG:'Consumer',ABNB:'Consumer',UBER:'Consumer',MAR:'Consumer',HLT:'Consumer',
  WMT:'Consumer',COST:'Consumer',TGT:'Consumer',KR:'Consumer',PG:'Consumer',
  KO:'Consumer',PEP:'Consumer',MO:'Consumer',MDLZ:'Consumer',GIS:'Consumer',
  DIS:'Consumer',WBD:'Consumer',SPOT:'Consumer',
  RTX:'Industrials',LMT:'Industrials',BA:'Industrials',NOC:'Industrials',GD:'Industrials',
  CAT:'Industrials',HON:'Industrials',GE:'Industrials',EMR:'Industrials',ETN:'Industrials',
  UPS:'Industrials',FDX:'Industrials',DE:'Industrials',ODFL:'Industrials',
  PLD:'Real Estate',AMT:'Real Estate',EQIX:'Real Estate',SPG:'Real Estate',
  O:'Real Estate',VICI:'Real Estate',PSA:'Real Estate',AVB:'Real Estate',
  FCX:'Materials',NEM:'Materials',DOW:'Materials',LIN:'Materials',
  APD:'Materials',SHW:'Materials',PPG:'Materials',ALB:'Materials',NUE:'Materials',
};
...
export default async function handler(req, res) {
  ...
  const cacheKey = `insider:v7:${days}`;

  try {
    let trades = refresh === 'true' ? null : await redisGet(cacheKey);

    if (!trades) {
      // Fetch latest Form 4 RSS from EDGAR
      const entries = await fetchEdgarRSS();

      // Filter by days
      const cutoff = new Date(Date.now() - parseInt(days) * 86400000);
      const recent = entries.filter(e => new Date(e.date) >= cutoff).slice(0, 35);

      // Parse each filing XML
      const parsed = await Promise.allSettled(
        recent.map(e => parseForm4(e.cik, e.accNo, e.date))
      );
      const raw = parsed.filter(r => r.status === 'fulfilled' && r.value).map(r => r.value);

      // Cluster + enrich + score
      const clusterMap = {};
